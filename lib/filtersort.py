...
class FilterSort():
    ...
    def make_sort(self, somedict):
        ...
        allowedsort = {k: v for k, v in args.items() if k in somedict.keys() and (v == 'asc' or v == 'desc')}
        return [ getattr(somedict[k], v) for k, v in allowedsort.items() ]
    ...
    def generate_next_sort(self, somelist):
        args = request.args
        allowedsort = {k: v for k, v in args.items() if k in somelist and (v == 'asc' or v == 'desc')}
